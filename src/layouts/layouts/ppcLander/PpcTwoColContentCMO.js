        ...
        <section className={`${cmo ? '' : 'pt-[60px]'} ${!audit ? 'md:pb-20' : 'md:pb-20'} ${audit && '!pt-20'} relative`}>
            ...
            <Container classes={`flex flex-col xl:flex-row gap-8 flex-nowrap justify-center items-center xl:items-start xl:p-0 pb-20 ${cmo ? '' : 'xl:pb-20'} `}>
                ...
                <div className="flex flex-col gap-6 flex-1">
                    {videoText?.heading &&
                        <h2 dangerouslySetInnerHTML={{__html: Parser(videoText.heading)}} className={`${videoText?.headingMobile ? 'hidden lgz:block' : ''} text-center xl:text-left max-w-[750px] mx-auto xl:ml-0 xl:max-w-full font-stratos uppercase text-40px leading-44px text-black`}></h2>
                    }
                    {videoText?.headingMobile &&
                        <h2 dangerouslySetInnerHTML={{__html: Parser(videoText.headingMobile)}} className={`lgz:hidden text-center xl:text-left max-w-[750px] mx-auto xl:ml-0 xl:max-w-full font-stratos uppercase text-40px leading-44px text-black`}></h2>
                    }
                    {videoText?.body &&
                        <p dangerouslySetInnerHTML={{__html: Parser(videoText.body)}} className={`${videoText?.bodyMobile ? 'hidden lgz:block' : ''} text-center xl:text-left font-basic-sans font-light !text-[21px] !leading-[28px] lg:!text-[26px] lg:!leading-[36px] text-rm-grey`}></p>
                    }
                    {videoText?.bodyMobile &&
                        <p dangerouslySetInnerHTML={{__html: Parser(videoText.bodyMobile)}} className={`lgz:hidden text-center xl:text-left font-basic-sans font-light !text-[21px] !leading-[28px] lg:!text-[26px] lg:!leading-[36px] text-rm-grey`}></p>
                    }
                    
                    {videoText?.video?.mediaItemUrl && 
                        <div className="relative max-w-[630px] xl:w-1/2 xl:hidden mx-auto">
                            ...
                            <svg className="absolute -right-[20px] -bottom-[65px] hidden lgz:block" width="132" height="131" viewBox="0 0 132 131" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                {/* Right Column - Video */}
                {videoText?.video?.mediaItemUrl && 
                    <div className="relative max-w-[630px] xl:w-1/2 hidden xl:block">
                        <video ref={video} className="aspect-video rounded-3xl flex-1 w-full " 
                                autoPlay                         
                                muted
                                loop

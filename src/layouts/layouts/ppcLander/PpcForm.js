...
const PPCForm = ({data, cmo, persistantEmail}) => {
    ...
    return(<>
        <section className="bg-white -mt-[1px]">
            ...
            <div className={`pb-[160px] relative ${cmo ? `pt-[160px] bg-[#f3f1ee] bg-[linear-gradient(225deg,rgba(0,171,182,0.25)_0%,transparent_35%,transparent_65%,rgba(0,171,182,0.25)_100%)]` : `bg-gradient-to-t from-white via-[#f3f9f9] via-47% to-[#edf8f9]` } z-20`}>
                <Container classes={`flex flex-col gap-[100px]`}>
                    <div className="flex flex-col-reverse items-center justify-center xl:flex-row xl:flex-wrap flex-nowrap xl:gap-6">
                        <div className="flex flex-col gap-6 text-left flex-1">
                            {heading &&
                                <h2 id="form" dangerouslySetInnerHTML={{__html: Parser(heading)}} className={`pt-20 text-center xl:text-left text-[2.5rem] leading-[2.7rem] md:text-[4.5rem] md:leading-[4.475rem] font-stratos font-semibold text-black uppercase`}></h2>
                            }
                            {body &&

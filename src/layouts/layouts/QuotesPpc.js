...
const QuotesPpc = (props) => {

    const content   = props.layoutData.layoutContent || {}
    const settings  = props.layoutData.layoutSettings || {}
    const slides    = content.quotes ? content.quotes : []
    const textColor = settings.backgroundColor === 'black' ? 'text-rm-white' : 'text-rm-aqua'
    const textColorSecondary = settings.backgroundColor === 'black' ? 'text-rm-white' : 'text-rm-black'
    ...
    return (
        <div ref={parallaxContainer} className="block relative">
            <Section settings={settings} classes="overflow-hidden">
                <Container container={settings.containerWidth}>
                    <div ref={sliderRef} className={`mt-12 flex w-full flex-wrap justify-between relative`}>
                        <div className={`p-8 lg:p-14 w-full`}>
                            <div key={Math.random()} className={`animate-quote text-center ${textColor}`}>
                                <div dangerouslySetInnerHTML={{__html: Parser(slidedata.content)}} className={ theme.text['Q'] + ' block transition-all ease-in-out font-normal' }></div>
                                <p className={ `${theme.text.P_BLD} ${textColorSecondary} pt-8 pb-2` }>
                                    { slidedata.title }
                                </p>
                                <small className={`${theme.text.FOOTER} ${textColorSecondary}`}>
                                    { slidedata?.reviewsFields?.titleCompany }
                                </small>
                            </div>
                        </div>
                        {arrows &&
                            <div className={`flex justify-center items-center bg-rm-pale-grey mx-auto text-black [&_path]:fill-current`}>
...
export default QuotesPpc


export const query = graphql`
  fragment QuotesPpcPage on WpPage_Flexiblelayouts_Layouts {
    ... on WpPage_Flexiblelayouts_Layouts_QuotesPpc {
      fieldGroupName
      layoutQuotesPpc {
        layoutContent {
          quotes {
...
export const serviceQuery = graphql`
  fragment QuotesPpcService on WpService_Flexiblelayouts_Layouts {
    ... on WpService_Flexiblelayouts_Layouts_QuotesPpc {
      fieldGroupName
      layoutQuotesPpc {
        layoutContent {
          quotes {
...
export const projectQuery = graphql`
  fragment QuotesPpcProject on WpProject_Flexiblelayouts_Layouts {
    ... on WpProject_Flexiblelayouts_Layouts_QuotesPpc {
      fieldGroupName
      layoutQuotesPpc {
        layoutContent {
          quotes {
...
export const landerQuery = graphql`
  fragment QuotesPpcLander on WpLander_Flexiblelayouts_Layouts {
    ... on WpLander_Flexiblelayouts_Layouts_QuotesPpc {
      fieldGroupName
      layoutQuotesPpc {
        layoutContent {
          quotes {
